export default class Permission {
  ...
  private requestWithCallback(onGranted: () => void, onDenied: () => void) {
    ...
    var resolved = false;
    var resolve = (result = this.win.Notification.permission) => {
      if (resolved) return;
      resolved = true;
    ...
    if (existing !== this.DEFAULT) {
      ...
      request = this.win.Notification.requestPermission(resolve);
   ...
   * @return {Promise}
   */
  private requestAsPromise(): Promise<void> {
    const existing = this.get();
    ...
    return new Promise((resolvePromise, rejectPromise) => {
      ...
      if (hasPermissions) {
        ...
        /* CHrome, instead, returns a Promise */
        request = this.win.Notification.requestPermission(resolver);
        if (request && request.then) {
          /* Chrome 23+ */
  ...
  get() {
    ...
    else if ((navigator as MozNavigator).mozNotification)
      /* Firefox Mobile */
