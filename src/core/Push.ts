...
export default class Push {
  ...
  constructor(win: {}) {
    ...
    this._configuration = {
      ...
      fallback: function(payload: object) {}
    };
  }
   ...
   * @private
   */
  _createCallback(title: string, options: PushOptions, resolve: Function) {
    let notification: GenericNotification = null;
    ...
    if (this._agents.desktop.isSupported()) {
      try {
        ...
        const cb = (notifications: GenericNotification[]) =>
          this._serviceWorkerCallback(notifications, options, resolve);
  ...
  create(title: string, options: {}): Promise<void> {
    ...
    if (!this.Permission.has()) {
      promiseCallback = (resolve: Function, reject: Function) => {
